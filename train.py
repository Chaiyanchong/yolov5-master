...
def load_weights(pretrained_weight, model):
    init_state_dict = model.state_dict()
    pretrained_state_dict = torch.load(pretrained_weight)
    if 'model' in pretrained_state_dict:
        pretrained_state_dict = pretrained_state_dict['model'].state_dict()

    for key in init_state_dict:
        print(key)
        if key in pretrained_state_dict and pretrained_state_dict[key].shape == init_state_dict[key].shape:
            init_state_dict[key] = pretrained_state_dict[key]
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='./models/yolov5-tiny_PPVD.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/PPVD.yaml', help='dataset.yaml path')
